...
// Maps new levels 1-9 to old level question_id prefixes
const LEVELS_SUMMARY = [
  { level: 1, oldPrefix: 'niveau1', title: 'Exploration des métiers', type: 'recherche' },
  { level: 2, oldPrefix: 'niveau11', title: 'Classement des domaines', type: 'classement' },
  { level: 3, oldPrefix: 'niveau3', title: 'Mythes du travail', type: 'quiz' },
  { level: 4, oldPrefix: 'niveau15', title: 'Avantages & inconvénients métier', type: 'analyse' },
  { level: 5, oldPrefix: 'niveau7', title: 'Compatibilité métier', type: 'évaluation' },
  { level: 6, oldPrefix: 'niveau12', title: 'Marché du travail', type: 'jeu' },
  { level: 7, oldPrefix: 'niveau21', title: 'Filières & notes', type: 'questionnaire' },
  { level: 8, oldPrefix: 'niveau22', title: 'Niveau d\'études & budget', type: 'questionnaire' },
  { level: 9, oldPrefix: 'niveau23', title: 'Recommandations d\'écoles', type: 'recherche' }
]

export default function NiveauBilanFinal() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        const prefixes = LEVELS_SUMMARY.map((l) => l.oldPrefix)
        const filtered = entries.filter((row) => {
          const id = String(row?.question_id || '').toLowerCase()
          return prefixes.some((p) => id.startsWith(p) || id.startsWith(p.replace('niveau', 'niveau_')))
        })
        setExtraInfos(filtered)
      } catch (e) {
        console.error('NiveauBilanFinal load error', e)
        if (!mounted) return
  ...
  async function loadBilan() {
    ...
    try {
      ...
      console.error('NiveauBilanFinal bilan fetch failed', e)
  ...
  const onFinish = async () => {
    ...
    try {
      ...
      await completeQuest('level_10')
      await usersAPI.saveExtraInfo([{
        question_id: 'bilan_final_diploma_date',
  ...
  const downloadBilan = async () => {
    ...
    try {
      ...
      doc.save('zelia-bilan-final.pdf')
    } catch (e) {
