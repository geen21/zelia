...
import { matchPersonaFromAnalysis, formatPersonaHeadline } from '../lib/personas.js'
import { AVATAR_3D_OPTIONS, getAvatar3dOption } from '../lib/avatar3d.js'
import { generatePersonaShareCard, deriveTopCompetences } from '../lib/shareImage.js'
...
export default function OrientationFlow() {
  ...
  const [avatarOptionId, setAvatarOptionId] = useState(() => localStorage.getItem('avatar_option_id') || AVATAR_3D_OPTIONS[0].id)
  const [avatarUrl, setAvatarUrl] = useState(() => localStorage.getItem('avatar_url') || getAvatar3dOption(localStorage.getItem('avatar_option_id'))?.url || AVATAR_3D_OPTIONS[0].url)
  const [mouthAlt, setMouthAlt] = useState(false)
  const [personaShareBusy, setPersonaShareBusy] = useState(false)
  ...
  const personaMatch = useMemo(
    () => matchPersonaFromAnalysis(analysisBlock, [], avatarOptionId),
    [analysisBlock, avatarOptionId]
  )
  ...
  const saveAvatarToProfile = useCallback(async () => {
    const avatarData = { id: avatarOptionId, url: avatarUrl, provider: 'fluent-emoji-3d' }
  ...
  }, [avatarOptionId, avatarUrl])

  const handleSharePersona = useCallback(async () => {
    const persona = personaMatch?.persona
    if (!persona || personaShareBusy) return
    setPersonaShareBusy(true)
    try {
      const competences = deriveTopCompetences(analysisBlock, 3)
      const dataUrl = await generatePersonaShareCard({
        persona,
        avatarUrl,
        domaines: persona.domaines,
        competences
      })
      if (!dataUrl) return
      const blob = await (await fetch(dataUrl)).blob()
      const file = new File([blob], `zelia-${persona.id}.png`, { type: 'image/png' })
      if (navigator.canShare && navigator.canShare({ files: [file] })) {
        await navigator.share({ files: [file], title: 'Mon profil Zélia', text: formatPersonaHeadline(persona) })
      } else {
        const link = document.createElement('a')
        link.href = dataUrl
        link.download = `zelia-${persona.id}.png`
        link.click()
      }
    } catch (shareError) {
      console.warn('Persona share failed', shareError)
    } finally {
      setPersonaShareBusy(false)
    }
  }, [personaMatch, personaShareBusy, analysisBlock, avatarUrl])
  ...
  const chooseAvatarOption = (option) => {
    setAvatarOptionId(option.id)
    setAvatarUrl(option.url)
    localStorage.setItem('avatar_option_id', option.id)
    localStorage.setItem('avatar_url', option.url)
  }

  const validateAvatarAndContinue = () => {
    localStorage.setItem('avatar_option_id', avatarOptionId)
  ...
  const renderAvatar = () => (
    <div className="orientation-stage compact">
      <div className="orientation-card avatar-card">
        <span className="orientation-pill">Avatar 3D</span>
        <h1>Choisis l'avatar qui te ressemble</h1>
        <img src={avatarUrl} alt="Avatar choisi" className="avatar-preview" />
        <div className="avatar3d-grid">
          {AVATAR_3D_OPTIONS.map((option) => (
            <button
              key={option.id}
              type="button"
              className={`avatar3d-option ${option.id === avatarOptionId ? 'active' : ''}`}
              onClick={() => chooseAvatarOption(option)}
              title={option.label}
            >
  ...
  const renderIntro = () => (
    <div className="orientation-stage compact">
      <div className="orientation-card message-card">
        {renderAvatarFace()}
        <span className="orientation-pill">Zélia</span>
        <h1>Salut, moi c'est Zélia.</h1>
        <p>
          Je vais te poser {QUESTION_LIMIT} questions, puis quelques infos rapides. Ensuite je te donne ta
          personnalité, tes forces, des idées de métiers et les études qui te correspondent.
        </p>
        <p>Prends le temps de réfléchir, il n'y a pas de mauvaise réponse. À toute&nbsp;!</p>
      </div>
  ...
  const renderPersonality = () => {
    const persona = personaMatch?.persona
    const competences = deriveTopCompetences(analysisBlock, 4)
    return (
      <div className="orientation-stage compact personality-stage">
        <div className="orientation-card recommendation-card personality-card persona-reveal-card">
          <img src={avatarUrl} alt="Ton avatar" className="persona-avatar" />
          <span className="orientation-pill">Analyse de personnalité</span>
          <h1>{persona ? formatPersonaHeadline(persona) : 'Voilà ce que ton profil raconte.'}</h1>
          {persona?.tagline && <p className="persona-tagline">{persona.tagline}</p>}
          {persona?.domaines?.length > 0 && (
            <div className="persona-chip-row">
              {persona.domaines.map((domaine) => <span key={domaine} className="persona-chip">{domaine}</span>)}
            </div>
          )}
          {competences.length > 0 && (
            <div className="persona-chip-row secondary">
              {competences.map((skill) => <span key={skill} className="persona-chip alt">{skill}</span>)}
            </div>
          )}
          {analysisBlock?.personalityAnalysis && (
            <p className="persona-analysis-text">{formatPersonalityAnalysisText(analysisBlock.personalityAnalysis)}</p>
          )}
          <button
            type="button"
            className="secondary-action persona-share-button"
            onClick={handleSharePersona}
            disabled={personaShareBusy}
          >
            <i className="ph ph-share-network" aria-hidden="true" />
            {personaShareBusy ? 'Préparation...' : 'Partager mon profil'}
          </button>
        </div>
        <button className="primary-action personality-continue" onClick={() => setPhase('intent')}>Découvrir mes pistes</button>
      </div>
    )
  }
